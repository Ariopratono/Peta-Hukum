...
class LegalRegulationPasal(models.Model):
    ...
    reference_ids = fields.Many2many(
        'legal.regulation.pasal',        # Target ke model Pasal juga
        'rel_pasal_references',
        'source_id', 'target_id',        # Column mapping
        string='Referensi Terkait',
        compute='_compute_reference_pasals',
        store=True
    )

    @api.depends('nomor_pasal', 'regulation_id') 
    def _compute_reference_pasals(self):
        for record in self:
            if not record.nomor_pasal: # Ubah 'content' jadi 'isi_pasal'
                record.reference_ids = False
                continue

            references = []
            pattern = r'Pasal\s+(\d+[A-Z]?)'
            
            # Ubah 'content' jadi 'isi_pasal'
            matches = re.findall(pattern, record.nomor_pasal)
            unique_matches = set(matches)

            if unique_matches:
                targets = self.search([
                    ('regulation_id', '=', record.regulation_id.id),
                    ('nomor_pasal', 'in', list(unique_matches)) 
                ])
                references.extend(targets.ids)
            
            record.reference_ids = [(6, 0, references)]
...
class LegalRegulationStructure(models.Model):
    _name = 'legal.regulation.structure'

    # Ubah field 'number' agar bisa menerima huruf (untuk Pasal 45A)
    # Catatan: Jika sebelumnya Integer, migrasi ke Char diperlukan.
    # Jika sudah Char, abaikan. Asumsi saya di kode awal Anda mungkin Integer/Char.
    # Sebaiknya gunakan Char untuk fleksibilitas.
    number_char = fields.Char(string='Nomor (Alfanumerik)', help="Contoh: 45A")

    # Status Pasal dalam Naskah Gabungan
    change_status = fields.Selection([
        ('original', 'Tetap/Asli'),
        ('amended', 'Diubah'),
        ('inserted', 'Disisipkan'),
        ('deleted', 'Dihapus')
    ], string='Status Perubahan', default='original')

    # Menyimpan info perubahan, misal: "Diubah oleh UU No. 19 Tahun 2016"
    change_reference = fields.Char(string='Dasar Perubahan') 
    
    # Relasi ke UU spesifik penyebab perubahan (opsional tapi bagus untuk data)
    amendment_source_id = fields.Many2one('legal.regulation', string='Sumber Perubahan')
