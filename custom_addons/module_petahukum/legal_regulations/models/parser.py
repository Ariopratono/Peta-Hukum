...
class PDFExtractor:
    ...
    def _get_structure_patterns(self):
        """ Update Regex untuk menangani Pasal Alfanumerik (45A) """
        patterns = super()._get_structure_patterns()
        
        # Update pola PASAL untuk menangkap angka diikuti huruf (opsional)
        # Contoh: Pasal 45A, Pasal 1
        # Regex lama mungkin: r'^Pasal\s+(\d+)'
        # Regex baru: r'^Pasal\s+(\d+[A-Za-z]*)'
        patterns['pasal'] = re.compile(r'^Pasal\s+(\d+[A-Za-z]*)', re.IGNORECASE)
        
        return patterns

    def parse_txt_content(self, regulation_id, content):
        """
        Override atau extend fungsi parsing untuk mendeteksi metadata perubahan
        dari file TXT gabungan yang diupload.
        """
        # Panggil logic parsing dasar (memecah Bab, Pasal, Ayat)
        # Kita asumsikan super() mengembalikan list of dict struktur
        parsed_structures = super().parse_txt_content(regulation_id, content)

        if regulation_id.regulation_type == 'consolidated':
            self._enrich_consolidated_data(parsed_structures, regulation_id)
        
        return parsed_structures

    def _enrich_consolidated_data(self, structures, regulation):
        """
        Logika tambahan untuk mendeteksi keyword perubahan pada naskah gabungan.
        Misal di teks ada: "Pasal 5 (Diubah oleh UU 19/2016)"
        """
        # Keyword regex
        re_changed = re.compile(r'\((Diubah|Disisipkan|Dihapus).*?(UU.*?)\)', re.IGNORECASE)
        
        for struct in structures:
            if struct.get('level') == 'pasal':
                text = struct.get('content', '')
                
                # Cek apakah ada tanda perubahan di teks pasal
                match = re_changed.search(text)
                if match:
                    action = match.group(1).lower() # diubah/disisipkan
                    source = match.group(2) # UU ...
                    
                    struct['change_reference'] = f"{action.title()} oleh {source}"
                    
                    if 'diubah' in action:
                        struct['change_status'] = 'amended'
                    elif 'disisipkan' in action:
                        struct['change_status'] = 'inserted'
                    elif 'dihapus' in action:
                        struct['change_status'] = 'deleted'
                        struct['content'] = '[Dihapus]' # Kosongkan konten jika dihapus
