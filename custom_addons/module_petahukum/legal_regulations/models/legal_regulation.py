...
class LegalRegulation(models.Model):
    ...
    # Menandakan apakah ini UU Perubahan
    is_amendment = fields.Boolean(string="Adalah UU Perubahan")

    # Relasi ke UU Induk (misal UU 19/2016 parent-nya UU 11/2008)
    original_regulation_id = fields.Many2one('legal.regulation', string="UU Induk")
    
    # Daftar semua perubahan (diisi di UU Induk)
    amendment_ids = fields.One2many('legal.regulation', 'original_regulation_id', string="Daftar Perubahan")

    is_consolidated = fields.Boolean(string="Naskah Gabungan")

    # Tambahan Field untuk Relasi & Tipe
    regulation_type = fields.Selection([
        ('original', 'Peraturan Asli/Tunggal'),
        ('amendment', 'Peraturan Perubahan'),
        ('consolidated', 'Naskah Kompilasi/Gabungan')
    ], string='Tipe Dokumen', default='original', required=True)

    # Relasi ke Peraturan Lain
    # Contoh: Jika ini UU 19/2016, maka parent_regulation_id adalah UU 11/2008
    parent_regulation_id = fields.Many2one(
        'legal.regulation', 
        string='Mengubah Peraturan',
        domain="[('regulation_type', 'in', ['original', 'consolidated'])]"
    )
    
    # Contoh: Jika ini UU 11/2008, maka amendment_ids adalah UU 19/2016, UU 1/2024
    amendment_ids = fields.One2many(
        'legal.regulation', 
        'parent_regulation_id', 
        string='Daftar Perubahan'
    )

    # Field untuk menyimpan referensi UU apa saja yang ada di dalam naskah gabungan ini
    source_regulation_ids = fields.Many2many(
        'legal.regulation',
        'regulation_consolidation_rel',
        'consolidation_id',
        'source_id',
        string='Sumber Peraturan (Gabungan)',
        help="Pilih UU Induk dan UU Perubahannya jika ini adalah Naskah Kompilasi"
    )

    description = fields.Html(string="Keterangan Perubahan", help="Misal: Perubahan Kedua atas UU ITE")
    ...
    def _extract_text_from_pdf(self, pdf_data):
        ...
        try:
            ...
            if text.strip():
                _logger.info(f"[DONE] Layout extraction SUCCESS: {len(text)} characters")
                _logger.info("First 500 chars of layout extraction:")
                _logger.info(text[:500])
            else:
                _logger.warning("[WARN] Layout extraction returned empty, falling back to auto extraction...")
...
''')
    ...
    def action_create_consolidation_pasal(self):
        """
        Fungsi untuk membuat 'Naskah Gabungan' dari UU Induk + Revisi.
        """
        self.ensure_one()
        if self.original_regulation_id:
            raise ValueError("Harap jalankan proses ini dari UU Induk (misal UU 11/2008).")

        # 1. Urutkan UU: Induk -> Perubahan 1 -> Perubahan 2 ... (Berdasarkan tahun/tanggal)
        # Tambahkan str() untuk memastikan tipe datanya sama-sama string
        all_regulations = [self] + list(self.amendment_ids.sorted(key=lambda r: str(r.tanggal_penetapan or r.tahun)))
        
        # 2. Siapkan Dictionary untuk menampung Pasal Final
        # Key: Nomor Pasal (misal "1", "27", "45A"), Value: Object Structure Terakhir
        consolidated_map = {}

        # 3. Iterasi dari UU terlama ke terbaru
        for reg in all_regulations:
            # Ambil semua pasal dari UU tersebut
            pasals = self.env['legal.regulation.pasal'].search([
                ('regulation_id', '=', reg.id),
                ('sequence', '=', 'pasal')
            ])
            
            for pasal in pasals:
                # Logika: Jika pasal sudah ada di consolidated_map, timpa dengan yang baru (Revisi).
                # Jika belum ada, tambahkan (Pasal Baru / Sisipan).
                # Jika statusnya "Dihapus" (perlu flag di structure), hapus dari map.
                
                # Kita gunakan field 'number' sebagai key unik (misal "27", "45A")
                if hasattr(pasal, 'status') and pasal.status == 'dihapus': 
                    if pasal.nomor_pasal in consolidated_map:
                        del consolidated_map[pasal.nomor_pasal]
                else:
                    consolidated_map[reg.nomor_pasal] = {
                        'source_uu': reg.id, # Untuk track ini pasal dari UU tahun berapa
                        'penjelasan': self.result_lines, 
                        'isi_pasal': pasal.isi_pasal
                    }

        # 4. Buat Record Baru "UU Gabungan"
        consolidated_name = f"Naskah Gabungan: {self.judul}"
        new_reg = self.create({
            'name': consolidated_name,
            'tahun': self.tahun,
            'number': self.nomor,
            'judul': self.judul if hasattr(self, 'judul') else self.name,
            'description': f"Gabungan dari {', '.join([r.judul for r in all_regulations])}",
            'is_consolidated': True, # Field baru boolean
        })

        # 5. Masukkan Struktur Pasal yang sudah digabung ke Record Baru
        # Kita perlu mengurutkan key (Nomor Pasal) agar urut: 1, 2, ... 27, 27A, 28
        sorted_keys = self._sort_pasal_numbers(list(consolidated_map.keys()))
        
        for nomor in sorted_keys:
            data = consolidated_map[nomor]
            self.env['legal.regulation.pasal'].create({
                'regulation_id': new_reg.id,
                'nomor_pasal': nomor,
                'isi_pasal': data['isi_pasal'],
                'name': f"Pasal {nomor}",
                # Tambahkan footer keterangan sumber, misal: [Diubah oleh UU 19/2016]
                'footnote': f"Sumber: {data['source_uu']}" 
            })
            
        return {
            'type': 'ir.actions.act_window',
            'judul': 'Naskah Gabungan',
            'view_mode': 'form',
            'res_model': 'legal.regulation',
            'res_id': new_reg.id,
        }
    
    def _sort_pasal_numbers(self, numbers):
        """
        Helper untuk mengurutkan ["1", "10", "2", "2A"] menjadi ["1", "2", "2A", "10"]
        """
        def sort_key(s):
            # Pisahkan angka dan huruf (misal 45A -> 45 dan A)
    ...
    def _extract_text_from_txt(self, txt_data):
        ...
        try:
            ...
            if 'Pasal' in text:
                try:
                    _logger.info("Detected Pasal, starting merging...")
                    text = self.action_create_consolidation_pasal(text)
                    _logger.info("Successfully merging pasals")
                except Exception as e:
                    _logger.error(f"Failed to merge pasal: {e}", exc_info=True)
                    _logger.warning(f"Using original text without merging pasal")
            else:
                _logger.info("No Penjelasan section found in text")
